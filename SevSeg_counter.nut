...
local displayType = 0;  // COMMON_CATHODE;  // Your display is either common cathode or common anode
...
local myDisplay = SevSeg(displayType, numberOfDigits, digit1, digit2, digit3, digit4, 255, 255, segA, segB, segC, segD, segE, segF, segG, segDP, 255, 255);

myDisplay.SetBrightness(100);  // Set the display to 100% brightness level

xtimer = millis();

local tempString;  // Used for sprintf
while (1) {
    // Example ways of displaying a decimal number
    tempString = format("%4d", deciSecond);  // Convert deciSecond into a string that is right adjusted
    //tempString = format("%d", deciSecond);  // Convert deciSecond into a string that is left adjusted
    //tempString = format("%04d", deciSecond);  // Convert deciSecond into a string with leading zeros
    //tempString = format("%4d", deciSecond * -1);  // Shows a negative sign infront of right adjusted number
    //tempString = format("%4X", deciSecond);  // Count in HEX, right adjusted
